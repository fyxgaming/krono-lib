...
export class FyxBlockchainPg implements IBlockchain {
    ...
    async broadcast(rawtx: string, mapiKey?: string) {
        ...
        await Promise.all([
            this.sql`INSERT INTO txns(id, rawtx)
                VALUES (${txidBuf}, ${tx.toBuffer()})
                ON CONFLICT DO NOTHING`,
            this.sql.unsafe(`UPDATE txos t
                SET spend_txid = decode('${txid}', 'hex')
                JOIN (VALUES ${spendValues}) as s(txid, vout) ON s.txid = t.txid AND s.vout = t.vout`),
            this.sql`INSERT INTO txos 
                ${this.sql(utxos, 'txid', 'vout', 'script', 'scripthash', 'satoshis')}
                ON CONFLICT DO NOTHING`,
