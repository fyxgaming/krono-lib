...
export class RestBlockchain {
    ...
    async jigIndex(address: string, kind = '', limit = 100, offset = 0) {
        const resp = await this.fetchLib(`${this.apiUrl}/jigs/address/${address}?limit=${limit}&offset=${offset}&kind=${kind || ''}`);
